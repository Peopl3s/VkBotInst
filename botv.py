'''
VkBot
An updated authorization system, in which you must specify the api version
'''
import vk
...
def getAttachment(typeContent, filename, pathToSave='C://Server/www/phv/'):
    ...
    if typeContent == "photo":
            try:
                placeUp = vkapi.photos.getMessagesUploadServer(version=5.68)
                ...
                savePhoto = vkapi.photos.saveMessagesPhoto(photo=upload['photo'], 
                                                           ...
                                                           hash=upload['hash'],version=5.68)[0]
        ...
        try:
            placeVideo = vkapi.video.save(is_private=1,version=5.68)
...
def loadInst(body, uid):
    delUnusedFiles()
    vkapi.messages.setActivity(type='typing', user_id=uid,version=5.68)    
    ...
    for newUrl in content:
        ...
        vkapi.messages.send(user_id=uid, message='Прямая сслыка: '+newUrl, 
                            attachment=attach,version=5.68)
...
def main():
    while True:
        delUnusedFiles()
        try:
            msg = vkapi.messages.get(version=5.68)[1:]
        except Exception as err:
            print(err)
            continue
        ...
        for message in msg:
            ...
            try:
                ...
                vkapi.messages.send(user_id=uid, message="Некорректная ссылка",version=5.68)
            finally:
                vkapi.messages.delete(message_ids=mid,version=5.68)
            sleep(4)
